...
func (um UserModel) Get(id uuid.UUID) (*User, error) {
	...
	err := um.DB.QueryRowContext(ctx, query, id).Scan(&user.ID,
		&user.Email, &user.Password.hash, &user.FirstName, &user.LastName, &user.IsActive, &user.Role, &user.Thumbnail, &user.DateJoined, &userP.ID, &userP.UserID, &userP.PhoneNumber, &userP.BirthDate,
	)
...
func (um UserModel) GetAll() ([]*User, error) {
	...
	for rows.Next() {
		var user User
		var userP UserProfile
		err := rows.Scan(&user.ID, &user.Email, &user.Password.hash, &user.FirstName, &user.LastName, &user.IsActive, &user.Role, &user.Thumbnail, &user.DateJoined, &userP.ID, &userP.UserID, &userP.PhoneNumber, &userP.BirthDate)
		if err != nil {
			return nil, err
		}
		user.Profile = userP
...
func (um UserModel) Update(user *User) (*User, error) {
	...
	RETURNING id, email, password, first_name, last_name, is_active, role, thumbnail, date_joined
	`
	...
	err = tx.QueryRowContext(ctx, query_user, args_user...).Scan(&userOut.ID,
		&userOut.Email, &userOut.Password.hash, &userOut.FirstName, &userOut.LastName, &userOut.IsActive, &userOut.Role, &userOut.Thumbnail, &userOut.DateJoined)
	...
	err = tx.QueryRowContext(ctx, query_user_profile, args_profile_user...).Scan(&userPOut.ID, &userPOut.UserID, &userPOut.PhoneNumber, &userPOut.BirthDate)
...
func (um UserModel) GetEmail(email string, active bool) (*User, error) {
	query := `
    SELECT 
        u.id, u.email, u.password, u.first_name, u.last_name, u.is_active, u.role, u.thumbnail, u.date_joined,
        p.id, p.user_id, p.phone_number, p.birth_date
    FROM 
        users u 
